        ...
        public override List<EntidadeDominio> consultar(EntidadeDominio entidade)
        {
            try
            {
                ...
                else
                {
                    if (Classe.Atual.ID == 0 && Classe.Passageiro.passagem.Voo.LO_chegada.ID == 0 && Classe.ID == 0)
                    {
                        sql = "SELECT bilhete.bilhete_id , IsDesembarque, id_status, bilhete.nome , atual , Departamento.nome as dep_nome,atual, pass_id,pass_lo_chegada ,pass_lo_partida  ,avi_id  ,data_partida ,data_chegada, b.nome p_nome,c.nome c_nome, b.sigla p_sigla,c.sigla c_sigla FROM status join bilhete using(bilhete_id) join viagem using(viagem_id) join passagens using(pass_id) join aeroporto b on(b.aero_id=pass_lo_partida) join aeroporto c on(c.aero_id=pass_lo_chegada) join Departamento on(atual=id_dep) WHERE id_dep!=0";
                    }
                    else if (Classe.Atual.ID == 0 && Classe.Passageiro.passagem.Voo.LO_chegada.ID != 0)
                    {
                        sql = "SELECT bilhete.bilhete_id , IsDesembarque, id_status, bilhete.nome , atual , Departamento.nome as dep_nome,atual, pass_id,pass_lo_chegada ,pass_lo_partida  ,avi_id  ,data_partida ,data_chegada, b.nome p_nome,c.nome c_nome, b.sigla p_sigla,c.sigla c_sigla FROM status join bilhete using(bilhete_id) join viagem using(viagem_id) join passagens using(pass_id) join aeroporto b on(b.aero_id=pass_lo_partida) join aeroporto c on(c.aero_id=pass_lo_chegada) join Departamento on(atual=id_dep) WHERE (IsDesembarque=false AND pass_lo_partida=:codd) OR (IsDesembarque=true AND pass_lo_chegada= :codd) and id_dep!=0";
                    }
                    else if (Classe.Atual.ID != 0 && Classe.Passageiro.passagem.Voo.LO_chegada.ID == 0)
                    {
                        sql = "SELECT bilhete.bilhete_id , IsDesembarque, id_status, bilhete.nome , atual , Departamento.nome as dep_nome,atual, pass_id,pass_lo_chegada ,pass_lo_partida  ,avi_id  ,data_partida ,data_chegada, b.nome p_nome,c.nome c_nome, b.sigla p_sigla,c.sigla c_sigla FROM status join bilhete using(bilhete_id) join viagem using(viagem_id) join passagens using(pass_id) join aeroporto b on(b.aero_id=pass_lo_partida) join aeroporto c on(c.aero_id=pass_lo_chegada) join Departamento on(atual=id_dep) WHERE atual=:cod and id_dep!=0";
                    }
                    else if (Classe.ID != 0)
                    {
                        sql = "SELECT bilhete.bilhete_id , IsDesembarque, id_status, bilhete.nome , atual , Departamento.nome as dep_nome,atual, pass_id,pass_lo_chegada ,pass_lo_partida  ,avi_id  ,data_partida ,data_chegada, b.nome p_nome,c.nome c_nome, b.sigla p_sigla,c.sigla c_sigla FROM status join bilhete using(bilhete_id) join viagem using(viagem_id) join passagens using(pass_id) join aeroporto b on(b.aero_id=pass_lo_partida) join aeroporto c on(c.aero_id=pass_lo_chegada) join Departamento on(atual=id_dep) WHERE id_status = :dev";
                    ...
                    {
                        sql = "SELECT bilhete.bilhete_id , IsDesembarque, id_status, bilhete.nome , atual , Departamento.nome as dep_nome,atual, pass_id,pass_lo_chegada ,pass_lo_partida  ,avi_id  ,data_partida ,data_chegada, b.nome p_nome,c.nome c_nome, b.sigla p_sigla,c.sigla c_sigla FROM status join bilhete using(bilhete_id) join viagem using(viagem_id) join passagens using(pass_id) join aeroporto b on(b.aero_id=pass_lo_partida) join aeroporto c on(c.aero_id=pass_lo_chegada) join Departamento on(atual=id_dep) WHERE ((IsDesembarque=false AND pass_lo_partida=:codd) OR (IsDesembarque=true AND pass_lo_chegada= :codd))and atual=:cod and id_dep!=0 ";
                    }
                }
                pst = new NpgsqlCommand();
                pst.CommandText = sql;
